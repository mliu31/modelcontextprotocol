...
3. Extend `resources/read` responses to optionally include resource metadata (name, title, description, icons, annotations, size) and add `metadataOnly` parameter to support efficient metadata-only reads (SEP-TBD).
